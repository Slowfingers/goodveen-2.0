              ...
              {step === 'payment' && (
                <SectionBlock title="Оплата">
                  <div className="flex flex-col gap-3">
                    ...
                    />
                    <DeliveryOption
                      icon={<Smartphone size={20} strokeWidth={1.25} />}
                      label="Apple Pay / Google Pay"
                      desc="Pay instantly from your device"
...
function deliveryLabel(d: Delivery) {
  return d === 'pickup'
    ? 'Pickup from atelier'
    : d === 'courier-fast'
    ? 'Express within 90 min'
    : 'Same-day courier';
}
function paymentLabel(p: Payment) {
  return p === 'card' ? 'Bank card' : p === 'apple' ? 'Apple/Google Pay' : 'Cash on delivery';
}
